	...
	context "logged in" do
	  before do
		...
		context "#member-list" do
			...
			it "contains all members each identified with data-user and a user class" do
				Factory.create(:user, :name=>"test member").set_canvas_role(@canvas, :member)
				render
				@canvas.members.each do |member|
					rendered.should have_selector("#member-list .user[data-user='#{member.user.id}']")
				end
			end
			...
			it "has a ban link associated with each member" do
				render
				@canvas.members.each do |member|
					rendered.should have_selector("#member-list .user[data-user='#{member.user.id}']") do |m|
						m.should have_link("ban")
					end
				end
			end
		end
